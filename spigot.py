import struct
from converting import unpack, bytetocodev1, bytetocodev2, bytetoip
class Spigot:
    def __init__(self):
        self.bytearr = []
    ...
    def drip(self, type, endian='<'):
        form = endian
        #print(type)
        if type not in ['string', 'packed', 'codev1', 'codev2', 'code', 'ip']:
    ...
    def wet(self, cusform):
        ...
        for char in form:
            ...
            elif char=='i': out.append(self.drip('ip'))
            elif char in '<>': endian = char
        return out
